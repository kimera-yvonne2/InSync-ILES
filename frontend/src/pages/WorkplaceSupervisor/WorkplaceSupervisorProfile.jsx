...
import { PW, PT, Card, Inp, GBtn, OBtn, LoadingSpinner, ErrorMsg, Label, Value } from "../../shared/ui";
import { useWorkplaceDashboard } from "../../hooks/useData";
import { authAPI } from "../../api/apiService";

export default function WPProfilePage() {
  const { data: me, loading, error } = useWorkplaceDashboard();
  ...
  const initials = `${me?.first_name?.[0] || ''}${me?.last_name?.[0] || ''}`.toUpperCase() || 'WS';
      ...
      <div className="grid grid-cols-4 gap-4 max-w-3xl">
        <div className="flex flex-col gap-3">
          <Card cls="text-center !py-8">
            ...
            <div className="mt-2 inline-block text-[10px] text-amber-400 border border-amber-500/30 rounded-full px-2 py-0.5 uppercase tracking-wider">WP Supervisor</div>
          </Card>
          <Card cls="!p-2">
          ...
          {tab === 'info' ? (
            <div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4">
                ...
                <div><Label>Role</Label><Value>Workplace Supervisor</Value></div>
              </div>
            </div>
          ) : (
            <div>
