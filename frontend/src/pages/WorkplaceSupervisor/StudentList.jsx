...
import { useWPStudents } from "../../hooks/useData";

export default function WPStudentsPage() {
  const { data: studRaw, loading, error } = useWPStudents();
  ...
  return (
    <PageWrap>
      ...
      {students.length === 0
        ? <EmptyState message="No students assigned to your supervision yet." />
        : (
          <Card style={{ padding: 0, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ background: COLORS.navyLight }}>
                  {['Student', 'Email', 'Programme', 'Year', 'Status'].map(h => (
              ...
              <tbody>
                {students.map((s, i) => (
                  <tr key={s.id} style={{ borderTop: `1px solid ${COLORS.navyBorder}`, background: i % 2 === 0 ? 'transparent' : '#0E1828' }}>
                    ...
                    <td style={{ padding: '14px 16px', fontSize: 12, color: COLORS.mutedLight }}>{s.year_of_study || '—'}</td>
